...
resource "random_id" "workload_publisher_client_id" {
  byte_length = 12
}

resource "random_password" "workload_publisher_client_secret" {
  length  = 24
  special = false
}
...
resource "random_id" "finops_agent_client_id" {
  byte_length = 12
}

resource "random_password" "finops_agent_client_secret" {
  length  = 24
  special = false
}
